      ...
      return (
        <div className="w-full h-screen bg-[#f3e9dc] flex items-center justify-center p-6 text-[#5d4037]">
          <div className="max-w-lg w-full bg-[#fffdfa] rounded-[2.5rem] border-[8px] border-[#d4af37] border-double shadow-2xl p-8 text-center">
            ...
            <h2 className="text-2xl font-black mb-3">앱이 잠깐 오류로 멈췄어요</h2>
            <p className="text-sm opacity-80 leading-relaxed mb-6">
              태블릿 브라우저(특히 Safari/WebView)에서 가끔 렌더링/오디오/메모리 문제로 런타임 에러가 나면 흰 화면이 됩니다.
              새로고침하면 대부분 복구됩니다.
            </p>
            <button
    ...
    <div className="w-full">
      <div className="flex justify-between items-end mb-1">
        <span className="text-[11px] font-black text-[#a67c52] uppercase tracking-tighter flex items-center gap-1">
          {label}{' '}
          {threshold && (
            isQualified ? <CheckCircle2 size={10} className="text-emerald-600" /> : <StarHalf size={10} className="text-rose-400" />
          )}
        </span>
      ...
      <div className="h-2.5 bg-white rounded-full border border-[#d4af37]/20 overflow-hidden relative shadow-inner">
        <div className={`h-full ${color} rounded-full transition-all duration-1000`} style={{ width: `${percent}%` }}>
          <div className="absolute inset-0 bg-white/30 progress-shimmer" />
        </div>
      </div>
    </div>
  );
...
const Soongyi = ({ size = 'text-[60px]', className = '' }) => (
  <div className={`relative inline-block leading-none select-none ${size} ${className}`}>
    🐒
    <span className="absolute -bottom-1 -right-2 text-[0.5em] drop-shadow-sm rotate-12">🎀</span>
  </div>
);
...
const CherryBlossomPetal = ({ style, className }) => (
  <div style={style} className={className}>
    <svg
      viewBox="0 0 100 100"
      fill="#D4BFFF"
      className="w-full h-full drop-shadow-[0_2px_3px_rgba(0,0,0,0.2)] opacity-70"
    >
...
const INTRO_STEPS = [
  { text: 'My name is Soongsoongyi. I am destined for the Royal Court.', emote: '🎀🐒' },
  { text: 'Prince Soongyi awaits me in London for the Grand Ball. I miss him dearly.', emote: '🐒✈️' },
  { text: 'Prince Soongyi welcomes princesses with refined English skills.', emote: '🐒📚' },
  { text: '7 days remain. I must transform into a true princess.', emote: '🐒✨' },
  { text: 'Royal Tutor, let the magnificent symphony of my life begin.', emote: '👑🐒' }
];
...
function AppInner() {
  ...
  useEffect(() => {
    return () => {
      // hard cleanup on unmount: tablet에서 꼬임 방지
      try { clearAllTimeouts(); } catch (_) {}
      try {
  ...
  const playSfx = useCallback((type) => {
    ...
    if (type === 'correct') {
      const harpNotes = [523.25, 659.25, 783.99, 1046.5, 1318.51, 1567.98, 2093.0];
      ...
      const gain = ctx.createGain();
      osc.type = 'sine';
      [440, 554, 659, 880].forEach((freq, i) => osc.frequency.setValueAtTime(freq, now + i * 0.1));
      gain.gain.setValueAtTime(0.1, now);
  ...
  const tryLoadAndPlay = useCallback(async (url) => {
    ...
    return await new Promise((resolve) => {
      ...
      try {
        ...
        // 일부 브라우저는 이벤트 오기 전에 play 시도하면 성공하는 경우가 있음
        try {
          const p = a.play();
          if (p && typeof p.then === 'function') {
  ...
  const handleStartInteraction = useCallback(async () => {
    ...
    if (!isAudioInitialized) {
      ...
      // iOS 정책: 사용자 제스처 시점에 오디오를 한번 깨워둬야 나중에 안정적
      if (bgmRef.current) {
        try {
  ...
  const applyAction = useCallback((act, count) => {
    ...
    setStats(prev => {
      ...
      if (curDay >= MAX_DAYS) pendingEndRef.current = { nextStats };
      return nextStats;
    });

    setLog(p => [`[DAY ${curDay}] ${act.name} completed.`, ...p.slice(0, 15)]);

    if (curDay < MAX_DAYS) setDay(d => Math.min(MAX_DAYS, d + 1));
  }, []);
  ...
  return (
    <div className="w-full h-screen bg-[#f3e9dc] flex items-center justify-center font-serif overflow-hidden p-2 relative selection:bg-[#d4af37]/30 text-[#5d4037]">
      ...
      <style>{`
        ...
        @keyframes fall { 
          0% { transform: translateY(-5vh) rotate(0); opacity: 0; } 
          20% { opacity: 0.7; } 
        ...
        .gold-shimmer { 
          background: linear-gradient(90deg, #8b6508, #daa520, #ffd700, #daa520, #8b6508); 
          background-size: 200% auto; 
          animation: shimmer 3s linear infinite; 
        }
        @keyframes shimmer { 0% { background-position: -200% center; } 100% { background-position: 200% center; } }

        .progress-shimmer { animation: shimmer 2s linear infinite; }
        ...
        @keyframes coupled-sway { 0%, 100% { transform: rotate(-5deg); } 50% { transform: rotate(5deg); } }
        @keyframes waltz-rise-fall { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-30px); } }
        ...
        .animate-gift { animation: gift-pop 1.2s infinite ease-in-out; }

        .rainbow-text {
          background: linear-gradient(90deg, #ff00ff, #00f2ff, #facc15, #ff0033, #bc13fe);
      ...
      {showBonusGift && (
        <div className="absolute inset-0 z-[1000] flex flex-col items-center justify-center bg-black/50 backdrop-blur-md transition-opacity duration-500">
          <div
            ...
            className="text-[160px] animate-gift drop-shadow-[0_30px_60px_rgba(0,0,0,0.4)] cursor-pointer hover:scale-110 active:scale-95 transition-transform select-none mb-6"
          >
            🎁
          </div>
      ...
      <div className="w-full max-w-5xl bg-[#fffdfa] rounded-[3.5rem] border-[12px] border-[#d4af37] shadow-[0_40px_80px_rgba(0,0,0,0.25)] flex flex-col relative h-full max-h-[95vh] border-double overflow-hidden z-10">
          ...
          <div
            ...
            onClick={() => {
              ...
              setLogoClickCount(prev => {
                const n = prev + 1;
                if (n >= 10) setImperialSparkles(true);
                return n;
              });
            }}
          >
            <Crown size={24} className="animate-pulse" />
            <span className={`text-[14px] font-black tracking-[0.3em] uppercase italic ${imperialSparkles ? 'rainbow-text' : ''}`}>
              Royal Ball Deluxe v9.8 {imperialSparkles && '✨'}
            </span>
          </div>

          <button
            onClick={() => { playSfx('click'); setShowTutorial(true); }}
            className="bg-white/20 hover:bg-white/40 px-5 py-1.5 rounded-full text-white text-[12px] font-bold flex items-center gap-2 transition-all"
          >
        ...
        <div className="flex-1 flex flex-col bg-[#fdf5e6] overflow-hidden relative">
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 relative overflow-hidden h-full z-[20]">
              <div className="absolute inset-0 pointer-events-none">
                {petals.map(p => (
                  <CherryBlossomPetal
                    key={p.key}
                    className="absolute top-0 animate-fall w-6 h-6"
              ...
              <button
                ...
                className="z-20 gold-shimmer text-white px-16 py-5 rounded-full text-2xl font-black shadow-[0_10px_0_#6b4a06] hover:scale-105 active:translate-y-2 uppercase tracking-widest border-2 border-white/40"
              >
                Start Protocol
              </button>
            </div>
          ...
          {screen === 'intro' && (
            <div className="flex-1 flex flex-col items-center justify-center p-6 bg-[#fdf5e6] z-[20] relative text-center">
              ...
              <div className="w-full max-w-xl bg-white/90 backdrop-blur-md rounded-[3rem] p-10 border-[6px] border-[#d4af37] border-double z-10 shadow-2xl relative">
                <p className="text-2xl font-serif font-bold text-[#5d4037] italic leading-relaxed">
                  "{INTRO_STEPS[introIdx].text}"
                </p>
                ...
                <div className="mt-10 flex justify-end">
                  <button
                    onClick={() => {
                      playSfx('click');
                      introIdx < 4 ? setIntroIdx(introIdx + 1) : setScreen('play');
                    }}
            ...
            <div className="flex-1 flex flex-col md:flex-row p-6 gap-6 bg-[#faf3e0] relative z-[20] overflow-hidden">
              <div className="w-full md:w-72 flex flex-col border-4 border-[#d4af37]/30 rounded-[2.5rem] shadow-xl shrink-0 h-full bg-[#f5efe1]/90 p-5 overflow-y-auto custom-scrollbar">
                ...
                <button
                  ...
                  className="bg-[#d32f2f] text-white py-2.5 rounded-2xl font-black uppercase text-[11px] flex items-center justify-center gap-2 hover:bg-red-700 transition-colors shadow-md active:translate-y-1 mb-6"
                >
                ...
                <div className="flex-1 bg-[#3d2b1f] p-4 rounded-3xl border-2 border-[#c19a6b]/40 shadow-inner flex flex-col min-h-[150px] overflow-hidden">
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar text-white/90 text-[11px] italic leading-relaxed">
                    {log.map((l, i) => (
                      <div key={i} className="mb-2 border-l-2 border-[#ffd700]/40 pl-2">{l}</div>
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 bg-white/60 border-4 border-[#d4af37]/30 rounded-[3.5rem] p-8 backdrop-blur-lg shadow-2xl flex flex-col overflow-hidden">
                <div className="flex justify-between items-center mb-8 border-b-4 border-double border-[#d4af37]/30 pb-4 flex-wrap gap-2">
                  ...
                  <div className="text-[12px] font-bold text-[#8b4513] animate-pulse uppercase">
                    Target: Int 90 | Flu 90 | Ene 50
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 overflow-y-auto pr-2 custom-scrollbar flex-1 pb-4">
                  {[
                    ...
                    <button
                      ...
                      className="bg-white/90 p-6 rounded-[3rem] border-4 border-transparent hover:border-[#d4af37] text-center shadow-xl active:scale-95 group transition-all flex flex-col items-center justify-center"
                    >
                      <div className="p-4 bg-[#fdf5e6] rounded-[1.5rem] mb-4 text-[#d4af37] group-hover:scale-110 transition-transform shadow-sm">
                        {act.icon}
                      </div>
          ...
          {currentQuizIdx >= 0 && (
            <div className="absolute inset-0 bg-black/85 backdrop-blur-xl z-[200] flex items-center justify-center p-4">
              <div className="bg-[#fffdfa] rounded-[3.5rem] border-[8px] border-[#d4af37] w-full max-w-lg relative shadow-2xl p-8 border-double">
                ...
                {wrongFeedback && (
                  <div className="absolute inset-0 bg-red-900/95 z-50 flex flex-col items-center justify-center p-8 text-white rounded-[3.5rem]">
                    <AlertTriangle size={64} className="animate-bounce mb-4" />
                    <h2 className="text-4xl font-black uppercase italic">TRY AGAIN!</h2>
                  </div>
                )}

                <h2 className="text-xl font-black text-center text-[#5d4037] mb-4 border-b-4 border-double border-[#d4af37]/20 pb-2 flex items-center justify-center gap-3">
                  <Music size={20} className="text-[#d4af37]" /> STEP {currentQuizIdx + 1}/4
                </h2>

                <div className="bg-[#fdf5e6] p-6 rounded-[2rem] text-center mb-6 shadow-inner border-2 border-[#d4af37]/10">
                  <p className="text-xl font-bold italic text-gray-700">"{quizQueue[currentQuizIdx]?.q}"</p>
                </div>
          ...
          {screen === 'ball' && (
            <div className="flex-1 flex flex-col items-center justify-center p-4 bg-[#1a1a2e] z-[20] relative overflow-y-auto">
              <div className="text-center z-10 w-full max-w-xl bg-white/5 backdrop-blur-2xl rounded-[3rem] p-8 border-4 border-[#d4af37] shadow-2xl">
                <div className="text-[60px] mb-6 flex justify-center gap-4 animate-bounce">👸👑<Soongyi size="text-[60px]" /></div>
                <h2 className="text-3xl royal-title text-[#ffd700] mb-6 border-b-2 border-[#d4af37]/30 pb-2">The Royal Conversation</h2>

                <div className="bg-white/10 p-6 rounded-[2rem] mb-6 border border-white/10 shadow-inner">
                  <p className="text-lg text-white font-serif italic leading-snug">"{BALL_DIALOGUES[ballStep]?.prince}"</p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-3 w-full">
                  {shuffledBallOptions.map((opt, i) => (
                    <button
                      key={`${ballStep}-${i}`}
                      onClick={() => handleBallChoice(opt.type)}
                      className="bg-white/5 hover:bg-[#d4af37] text-white hover:text-black py-4 px-6 rounded-[1.2rem] font-black text-sm transition-all border border-[#d4af37]/30 group flex items-center justify-between text-left"
                    >
                      <span>{opt.text}</span>
          ...
          {screen === 'dance-choice' && (
            <div
              className="flex-1 flex flex-col items-center justify-center p-4 z-[20] relative bg-cover bg-center"
              style={{ backgroundImage: "url('https://storage.googleapis.com/eunyeongsmusicfiles/beautiful-castle-architecture.jpg')" }}
            >
              ...
              <div className="w-full max-w-2xl bg-white/95 backdrop-blur-xl rounded-[2.5rem] border-[6px] border-double border-[#d4af37] p-8 z-20 shadow-2xl text-center">
                <p className="text-xl font-bold text-[#5d4037] mb-8 leading-relaxed">
                  "Prince Soongyi wants to dance with Soongsoongyi. Which music would you choose for them?"
                </p>

                <div className="grid grid-cols-1 gap-4">
                  <button
                    onClick={() => handleDanceMusic('classical')}
                    className="bg-[#d4af37] text-white py-4 rounded-2xl font-black hover:bg-[#b8860b] transition-all text-lg shadow-lg border-2 border-white/20"
                  >
                    Classical Waltz music
                  </button>

                  <button
                    onClick={() => handleDanceMusic('hiphop')}
                    className="bg-[#d4af37] text-white py-4 rounded-2xl font-black hover:bg-[#b8860b] transition-all text-lg shadow-lg border-2 border-white/20"
                  >
                    Hip hop music
                  </button>
                  ...
                  <button
                    onClick={() => handleDanceMusic('reject')}
                    className="bg-[#d4af37] text-white py-4 rounded-2xl font-black hover:bg-[#b8860b] transition-all text-lg shadow-lg border-2 border-white/20"
                  >
                    Soongsoongyi rejects his offer to dance.
                  </button>
                </div>
              </div>
            </div>
          ...
          {screen === 'performance-waltz' && (
            <div className="flex-1 relative overflow-hidden bg-black flex items-center justify-center">
              <div
                ...
                style={{
                  backgroundImage:
                    "linear-gradient(rgba(0, 0, 0, 0.2), rgba(0, 0, 0, 0.2)), url('https://storage.googleapis.com/eunyeongsmusicfiles/beautiful-castle-architecture.jpg')",
                  backgroundSize: 'cover',
                  backgroundPosition: 'center'
                }}
              />
              <div className="absolute inset-0 z-2 pointer-events-none" style={{ background: 'radial-gradient(circle at center, rgba(255, 255, 255, 0.1) 0%, rgba(0, 0, 0, 0.3) 100%)' }} />

              {!isPerforming ? (
                <div
                  onClick={() => startActualPerformance('waltz')}
                  className="z-[100] bg-white/95 p-12 rounded-[5rem] cursor-pointer text-center shadow-2xl border-4 border-[#d4af37] hover:scale-105 transition-transform"
                >
                  <div className="text-3xl font-black text-gray-800">Start Performance 🎵</div>
                  ...
                  <div className="couple-container animate-[zigzag-dance_15s_infinite_linear]">
                    <div className="dance-pivot relative flex items-center justify-center gap-12 animate-[coupled-sway_4s_infinite_ease-in-out]">
                      <div className="flex flex-col items-center drop-shadow-2xl animate-[waltz-rise-fall_3s_infinite_ease-in-out] delay-0">
                        <div className="text-[80px] mb-[-40px] z-50">👑</div>
                      ...
                      <div className="flex flex-col items-center drop-shadow-2xl animate-[waltz-rise-fall_3s_infinite_ease-in-out] delay-500">
                        <div className="text-[150px] relative">
                          🐒
                          <div className="absolute bottom-[20px] right-[-20px] text-[60px] rotate-[-15deg] z-50">🎀</div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div className="absolute top-10 left-1/2 -translate-x-1/2 z-[100] bg-white/90 px-8 py-3 rounded-full font-black shadow-xl text-[#4a148c] animate-pulse">
                    Waltz: {performanceTimer}s
                  </div>
                </div>
              )}
            </div>
          ...
          {screen === 'performance-hiphop' && (
            <div className="flex-1 relative overflow-hidden bg-black flex flex-col items-center justify-center">
              <div
                className="absolute inset-0 z-1"
                style={{ backgroundImage: "url('https://storage.googleapis.com/eunyeongsmusicfiles/beautiful-castle-architecture.jpg')", backgroundSize: 'cover', backgroundPosition: 'center' }}
              />
              <div className="absolute inset-0 z-2 pointer-events-none bg-black/40"></div>

              {!isPerforming ? (
                <div
                  onClick={() => startActualPerformance('hiphop')}
                  className="z-[100] bg-white/95 p-12 rounded-[5rem] cursor-pointer text-center shadow-2xl border-4 border-[#d4af37] hover:scale-105 transition-transform"
                >
                  <div className="text-3xl font-black text-gray-800">Start Performance 🎵</div>
                ...
                <div className="relative z-10 flex justify-around items-end w-full max-w-4xl h-[500px]">
                  <div className="monkey-group dancing flex flex-col items-center relative">
                    <div className="text-[80px] mb-[-40px] z-20">👑</div>
                    <div className="text-[180px]">🐒</div>
                  </div>

                  <div className="monkey-group dancing flex flex-col items-center relative">
                    <div className="text-[180px]">🐒</div>
                    <div className="absolute bottom-[25px] right-[-35px] text-[70px] z-20">🎀</div>
                  </div>

                  <div className="absolute top-0 left-1/2 -translate-x-1/2 z-[100] bg-white/90 px-8 py-3 rounded-full font-black shadow-xl text-[#4834d4] animate-pulse">
                    Hip Hop: {performanceTimer}s
                  </div>
                </div>
              )}
            </div>
          ...
          {['success', 'neutral', 'fail'].includes(screen) && (
            <div className="flex-1 flex flex-col items-center p-10 text-center bg-white z-[20] overflow-y-auto custom-scrollbar h-full">
              ...
              <button
                onClick={handleRestartGame}
                className="gold-shimmer text-white px-20 py-6 rounded-full font-black text-3xl shadow-[0_10px_0_#4a3705] transition-all uppercase border-4 border-white/40 active:translate-y-2 mb-8 flex-shrink-0"
              >
                Restart Protocol
              </button>
            </div>
      ...
      {showTutorial && (
        <div className="absolute inset-0 bg-black/98 z-[400] flex items-center justify-center p-4 backdrop-blur-3xl font-serif">
          <div className="bg-[#fffdfa] rounded-[5rem] border-[14px] border-[#c19a6b] w-full max-w-4xl p-8 relative shadow-2xl h-[90vh] flex flex-col border-double overflow-hidden">
            <button
              onClick={() => { playSfx('click'); setShowTutorial(false); }}
              className="absolute top-6 right-6 p-3 text-gray-400 hover:text-rose-500 transition-all z-10"
            >
              <X size={36} />
            </button>

            <h2 className="text-4xl md:text-5xl font-black text-[#8b4513] uppercase mb-8 border-b-4 border-[#c19a6b]/30 pb-4 flex items-center gap-6">
            ...
            <div className="flex-1 overflow-y-auto space-y-12 custom-scrollbar px-6 text-xl text-gray-700 leading-relaxed pb-8">
              <section className="space-y-4">
                ...
                <p className="italic bg-[#fdf5e6] p-6 rounded-2xl border border-[#d4af37]/20 shadow-inner">
                  Esteemed Tutor, your noble mission is to refine Soongsoongyi into a paragon of the high imperial society within 7 suns.
                  Every academic pursuit, every piece of literature read, and every etiquette choice made will echo through the majestic halls of the Empire.
                  Do not falter, for the Prince's heart and the future of the dynasty await your guidance. Eunyeong teacher also wishes you peace and success.
                </p>
              </section>
              ...
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 font-black text-center">
                  <div className="bg-indigo-50 p-6 rounded-3xl border-2 border-indigo-100 shadow-sm">
                    <div className="text-indigo-800 text-3xl">Int 90+</div>
                    <div className="text-xs uppercase mt-2 font-bold italic">Mastery of Literature</div>
                  </div>
                  <div className="bg-emerald-50 p-6 rounded-3xl border-2 border-emerald-100 shadow-sm">
                    <div className="text-emerald-800 text-3xl">Flu 90+</div>
                    <div className="text-xs uppercase mt-2 font-bold italic">Rhetorical Excellence</div>
                  </div>
                  <div className="bg-rose-50 p-6 rounded-3xl border-2 border-emerald-100 shadow-sm">
                    <div className="text-rose-800 text-3xl">Ene 50+</div>
                    <div className="text-xs uppercase mt-2 font-bold italic">Noble Stamina</div>
                  </div>
                </div>

                <div className="p-5 bg-rose-50 border-2 border-rose-100 rounded-2xl text-rose-800 text-base leading-relaxed">
                  <p className="font-bold flex items-center gap-2 mb-2">
                    <AlertTriangle size={20} /> ROYAL WARNINGS:
                  </p>
              ...
              <section className="space-y-4">
                ...
                <p className="italic bg-[#f3e9dc] p-6 rounded-2xl border border-[#d4af37]/20 shadow-inner">
                  On the 8th day, the Grand Ball commences. Your conversational choices with the Prince are paramount.
                  To achieve the <strong>Success Ending</strong>, you must choose the 'Elegant' response in all 4 dialogues.
                  A single lapse in social grace will lead to a Neutral fate, while two or more clumsy remarks will result in a tragic Failure.
                </p>
              </section>
            </div>

            <button
              onClick={() => { playSfx('start'); setShowTutorial(false); }}
              className="mt-4 bg-[#5d4037] text-white py-6 rounded-[3rem] font-black text-3xl uppercase shadow-2xl hover:bg-black active:scale-95 border-b-8 border-black transition-all"
            >
              I ACCEPT YOUR OFFER!
            </button>
          </div>
